...
function formIsValid() {
    ...
    let checkbox1 = new Field("defaultCheck1", "label1")
    let checkbox2 = new Field("defaultCheck2", "label2")
    let radio2 = new Field("exampleRadios2", "label4")

	let fields = [
        email, phone, password, passwordRepeat,
        firstName, surname, lastName, address, department
    ]
    
    let invalid = []

    if (!checkbox1.checked()) {
        invalid.push(checkbox1);
    }
    if (!checkbox2.checked()) {
        invalid.push(checkbox2);
    }
    if (!radio2.checked()) {
        invalid.push(radio2);
    }
